function [lb, ub, xf, utime, ltime] = dnn3p(W,P,lam)
...
cvx_begin sdp
    ...
    maximize ( 4*w0 + 8*wk'*X(2:(n+1),1) + 4*trace(W(U,U)*X(2:(n+1),2:(n+1))) ...
                - lam*trace(H1*X)) 
    X(1,1) == 1
    X(:) >= 0
...
tic;
%xl = bound.triv_bound(X(2:(n+1),1)*2 -1);
xl = bound.GW_lbound(2*X(1:(n+1),1:(n+1))+1,W(U,U),100);
