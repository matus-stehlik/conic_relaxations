function [lb, ub, xf] = sdp_slow_bnb(W,P)
...
cvx_begin sdp
    ...
    for i = 2:(n-1)
        for j = (i+1):(n-1)
            for k = (j+1):(n-1)
                if (W(U(i),U(j))~=0 && W(U(j),U(k))~=0 && W(U(k),U(i))~=0)
                    Y(i,j) + Y(j,k) + Y(i,k) >= -1
                    Y(i,j) - Y(j,k) - Y(i,k) >= -1
                  - Y(i,j) + Y(j,k) - Y(i,k) >= -1
                  - Y(i,j) - Y(j,k) + Y(i,k) >= -1
    ...
    vec(Y(2:end,2:end) + ones(n-1,1)*Y(2:end,1)' + Y(2:end,1)*ones(1,n-1) ) >=  -1
    vec(Y(2:end,2:end) + ones(n-1,1)*Y(2:end,1)' - Y(2:end,1)*ones(1,n-1) ) >=  -1
    vec(Y(2:end,2:end) - ones(n-1,1)*Y(2:end,1)' - Y(2:end,1)*ones(1,n-1) ) >=  -1
    end
    Y == semidefinite(n);
...
ub = ceil(cvx_optval)  ;  % from sdp we have obtained lower bound
